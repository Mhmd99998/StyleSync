...
class VariantService {
  ...
  async createVariant(variant: CreateVariantDto): Promise<AdminVariantDto> {
    try {
  ...
  async colorUsed(color: string): Promise<boolean> {
    try {
      const response = await axiosInstance.get(`${API_URL}/used`, {
        params: { color: color },
      });
      return response.data;
    } catch (error) {
      throw new Error("Failed to check color status");
    }
  }
}
